module.exports = (() => {
  ...
  class CommandLineInterface {
    ...
    run(args) {
      ...
      if (!command) {
        console.error(colors.bold.red('Error: ') + `Command "${this.format(args.commandName, args.subName)}" does not exist.`);
        return process.exit(1);
      }

      return command.run(args.args, args.flags, args.vflags, (err, result) => {
